...
uint16_t targetLoad = 111;
uint16_t holdLoad = 111;
int32_t posOpen = 2800;
int32_t posClose = 700;
void safeOpen() {
    dxl.write(MOTOR_ID, ADDR_GOAL_CURRENT, (uint8_t*)&holdLoad, 2);
...
void safeClose() {
    dxl.write(MOTOR_ID, ADDR_GOAL_CURRENT, (uint8_t*)&targetLoad, 2);
    ...
    while (millis() - startTime < 60000) { 
        ...
        dxl.read(MOTOR_ID, ADDR_PRESENT_LOAD, 2, (uint8_t*)&load, 10);
        bt.print(F("P:")); bt.print(pos);
        bt.print(F("|L:")); bt.print(load);
        bt.print(F("|V:")); bt.println(abs(vel));
        if (abs(load) >= 800 || (abs(vel) < 3 && abs(load) >= (targetLoad - 10))) {
            if (++stallCount > 1) { 
                dxl.write(MOTOR_ID, ADDR_GOAL_CURRENT, (uint8_t*)&holdLoad, 2);
...
void setup() {
    ...
    if (dxl.ping(MOTOR_ID)) {
        ...
        uint8_t m = 3;
        dxl.write(MOTOR_ID, ADDR_OPERATING_MODE, &m, 1);
